...
mkdirSync(new URL('./v49_31_screenshots/', import.meta.url), { recursive: true });
...
async function waitReady() {
  for (let i = 0; i < 40; i++) {
    const ok = await ev(`(() => !!(window.__tempBuilt && window.__latGradient && window.__landGJ))()`);
    if (ok) return true;
    await wait(400);
  }
  return false;
...
const run = async (width, height, lang, tag) => {
  ...
  const m = await ev(`(() => {
    ...
    const lats = [87, 73, 58, 43, 28, 13, 2, -2, -13, -28, -43, -58, -73, -87];
    const rows = lats.map(l => ({ lat: l, c: px(l, -120) }));
    return {
      w, h,
      tempBuilt: !!window.__tempBuilt,
      latGradient: !!window.__latGradient,
  ...
  writeFileSync(new URL('./v49_31_screenshots/index-' + tag + '.png', import.meta.url), Buffer.from(shot.data, 'base64'));
  return m;
};
...
writeFileSync(new URL('./v4931_check.json', import.meta.url), JSON.stringify(out, null, 2));

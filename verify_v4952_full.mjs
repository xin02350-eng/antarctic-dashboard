...
async function state() {
  return ev(`(() => {
    ...
    const sh = span.querySelector('.explore-shine');
    ...
    return {
      ...
      shineClip: getComputedStyle(sh).clipPath,
      shineAnim: getComputedStyle(sh).animationName,
      flowAnim: bCs.animationName,
      frame: {
        l: Math.round(b.left + parseFloat(aCs.left)),
        r: Math.round(b.right - parseFloat(aCs.right)),
        t: Math.round(b.top + parseFloat(aCs.top)),
        b: Math.round(b.bottom - parseFloat(aCs.bottom))
      }
    };
  })()`);
}
...
for (const lang of ['zh', 'en']) {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    const x = Math.max(0, Math.round(rect.x - 4));
    const y = Math.max(0, Math.round(rect.y - 6));
    const shot = await send('Page.captureScreenshot', { format: 'png', clip: { x, y, width: Math.round(rect.w + 8), height: Math.round(rect.h + 12), scale: 1 } });
    ...
    if (s.n > 20) {
      ...
      writeFileSync(`./v49_52_shots/${lang}_sweep_${i}.png`, Buffer.from(full.data, 'base64'));
    }
    await wait(70);
  }
  console.log('SCAN ' + lang + ' ' + JSON.stringify(scan));
}
ws.close();

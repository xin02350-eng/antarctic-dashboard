  ...
  const info = await ev(`(() => {
    ...
    function inScroller(el) {
      ...
      while (p) {
        ...
        if (st.overflowX === 'auto' || st.overflowX === 'scroll' || st.overflowX === 'hidden' || st.overflowY === 'auto' || st.overflowY === 'scroll' || st.overflowY === 'hidden') return true;
        p = p.parentElement;
      }
      return false;
    }
    ...
    for (let i = 0; i < els.length; i++) for (let j = i + 1; j < els.length; j++) {
      ...
      if (inter > 0.12 * minArea && minArea > 2000) overlaps.push(String(a.el.className).slice(0, 22) + '|' + String(b.el.className).slice(0, 22));
    }
    const over = [];
    els.forEach(o => {
      if (String(o.el.className).includes('leaflet-tile') || o.el.tagName === 'path' || o.el.tagName === 'svg') return;
      if ((o.r.right > cw + 1 || o.r.left < -1) && !inScroller(o.el) && o.s.position !== 'fixed') over.push(String(o.el.className).slice(0, 26) + ' R' + Math.round(o.r.right));
    });
    return { overlaps: overlaps.slice(0, 8), over: over.slice(0, 8) };
  })()`);
  const flag = (info.overlaps.length || info.over.length) ? ' !!' : ' ok';
